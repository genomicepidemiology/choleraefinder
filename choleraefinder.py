...
class CholeraeFinder(CGEFinder):
    ...
    def typer(self, json_results):
        """
        Reads the json_results to type the vibrio cholerae specie and prepares
        the results for the wrapper
        """
        prot_class = {}
        list_genes = []
        type_cholerae = {"specie": "Not able to determine",
                         "biotype": "Not able to determine",
                         "serogroup": "Not able to determine",
                         "wave": "Not able to determine",
                         "pandemic": "Not able to determine"}
        for i in json_results:
            if json_results[i][i.lower()] == "No hit found":
                prot_class[i] = []
            else:
                headers = json_results[i][i.lower()].keys()
                genes = []
                for head in list(headers):
                    gene = head.split(":")[0]
                    genes.append(gene)
                list_genes.extend(genes)
                prot_class[i] = genes
        if 'ompW' not in list_genes:
            type_cholerae["specie"] = "Not Vibrio Cholerae"
        else:
            type_cholerae["specie"] = "Vibrio Cholerae"
            if "rfbV_O1" in list_genes and "wbfZ_O139" not in list_genes:
                type_cholerae["serogroup"] = "O1"
                if "VC2346" in list_genes:
                ...
                if ("rstR_cc" in list_genes or "tcpA_cc" in list_genes) and ("rstR_et" not in list_genes and "tcpA_et3" not in list_genes):
                    ...
                    if "ctxB_1" in list_genes and type_cholerae["pandemic"]:
                        type_cholerae["biotype"] = "El Tor Variant"
                        type_cholerae["wave"] = "Early Wave 3"
                    elif "ctxB_3" in list_genes and type_cholerae["pandemic"]:
                        type_cholerae["biotype"] = "El Tor"
                        type_cholerae["wave"] = "Wave 1"
                    elif "ctxB_7" in list_genes and type_cholerae["pandemic"]:
                        type_cholerae["biotype"] = "El Tor Variant"
                        type_cholerae["wave"] = "Late Wave 3"
                    else:
                        type_cholerae["biotype"] = "El Tor"

                elif "rstR_cc" not in list_genes and "tcpA_cc" not in list_genes and "rstR_et" not in list_genes or "tcpA_et3" not in list_genes:
                    if type_cholerae["pandemic"]:
                        if "ctxB_1" in list_genes:
                            type_cholerae["biotype"] = "El Tor Variant"
                            type_cholerae["wave"] = "Early Wave 3"
                        elif "ctxB_3" in list_genes:
                            type_cholerae["biotype"] = "El Tor"
                            type_cholerae["wave"] = "Wave 1"
                        elif "ctxB_7" in list_genes:
                            type_cholerae["biotype"] = "El Tor Variant"
                            type_cholerae["wave"] = "Late Wave 3"
                    else:
                        if "ctxB_1" in list_genes:
                            type_cholerae["biotype"] = "Classical"
            elif "wbfZ_O139" in list_genes and "rfbV_O1" not in list_genes:
                type_cholerae["serogroup"] = "O139"
            elif "rfvB_O1" not in list_genes and "wbfZ_O139" not in list_genes:
                type_cholerae["serogroup"] = "No O1 / No O139"
            else:
                type_cholerae["serogroup"] = "Not able to determine. Both rfvB-O1 and wbfZ-O139 have been found. Please look at the Gene Description section to interpretate the results."

        return type_cholerae, prot_class
    ...
    def create_results(self, results_method, outdir):
        ...
        for db in results:
            ...
            if results[db] == "No hit found":
                ...
                for contig_id, hit in results[db].items():
                    ...
                    contig_res[contig_id].append([hit["query_start"],
            ...
            for contig_id, hit_lsts in contig_res.items():
                ...
                for hit in hits:
                    ...
                    positions_contig = "%s..%s" % (hit["query_start"],
                                                   hit["query_end"])
                    positions_ref = "%s..%s" % (hit["sbjct_start"],
                                                hit["sbjct_end"])
                    contig_name = hit["contig_name"]
                    description = self.phenos.get(gene, "Warning: gene is missing from "
        ...
        data[service]["typing_cholerae"] = type_cholerae
        ...
        header = ["Vibrio Cholerae Gene", "Identity", "Query / Template length",
        ...
        if args.extented_output:
            ...
            sbjct_filename = "{}/Cholerae_seqs.fsa".format(outdir)
            ...
            for species, dbs_info in json_results.items():
                for db_name, db_hits in dbs_info.items():
                    ...
                    result_file.write(self.description_dbs[species.lower()] + "\n")
                    db_rows = []
                    # Check it hits are found
                    if isinstance(db_hits, str):
                        content = [''] * len(header)
                        content[int(len(header) / 2)] = db_hits
                        result_file.write(CholeraeFinder.text_table(header, [content]) + "\n")
                        continue
                    for gene_id, gene_info in sorted(db_hits.items(),key=lambda x: (x[1]['vibrio_cholerae_gene'],x[1]['accession'])):
                        res_gene = gene_info["vibrio_cholerae_gene"]
                    ...
                    result_file.write(CholeraeFinder.text_table(header, db_rows) + "\n")
        ...
        for dbs_info in json_data.values():
            for db_name, db_info in dbs_info.items():
                ...
                for gene_id, gene_info in sorted(
                       db_info.items(),
                       key=lambda x: (x[1]['vibrio_cholerae_gene'],
                                      x[1]['accession'])):

                    seq_name = ("{gene}_{acc}"
                                .format(gene=gene_info["vibrio_cholerae_gene"],
                                        acc=gene_info["accession"]))
                    hit_name = gene_info["hit_id"]
    ...
    def load_notes(self, notes):
        with open(notes, 'r') as f:
            for line in f:
                ...
                self.phenos[tmp[0]] = "%s: %s: %s" % (tmp[1], tmp[2], tmp[3])
    ...
    def load_databases(self, databases):
        ...
        if databases is '':
            ...
            # Check that the CholeraeFinder DBs are valid
...
if __name__ == '__main__':
    ...
    # Check the number of files
        ...
        method = "kma"
        finder = CholeraeFinder(db_conf_file=db_config_file,
                                databases=args.databases, db_path=db_path,
                                db_path_kma=args.db_path, notes=notes_path)
        ...
        kma_run = finder.kma(inputfile_1=infile_1, inputfile_2=infile_2,
                             ...
                             kma_path=method_path, kma_add_args=extra_args)
